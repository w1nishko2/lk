                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select @error('type') is-invalid @enderror" 
                                            ...
                                            <option value="header" {{ old('type', $duplicateData['type'] ?? '') == 'header' ? 'selected' : '' }}>Header (Шапка)</option>
                                            <option value="hero" {{ old('type', $duplicateData['type'] ?? '') == 'hero' ? 'selected' : '' }}>Hero (Главный баннер)</option>
                                            <option value="features" {{ old('type', $duplicateData['type'] ?? '') == 'features' ? 'selected' : '' }}>Features (Возможности)</option>
                                            <option value="services" {{ old('type', $duplicateData['type'] ?? '') == 'services' ? 'selected' : '' }}>Services (Услуги)</option>
                                            <option value="about" {{ old('type', $duplicateData['type'] ?? '') == 'about' ? 'selected' : '' }}>About (О нас)</option>
                                            <option value="contact" {{ old('type', $duplicateData['type'] ?? '') == 'contact' ? 'selected' : '' }}>Contact (Контакты)</option>
                                            <option value="cta" {{ old('type', $duplicateData['type'] ?? '') == 'cta' ? 'selected' : '' }}>CTA (Призыв к действию)</option>
                                            <option value="footer" {{ old('type', $duplicateData['type'] ?? '') == 'footer' ? 'selected' : '' }}>Footer (Подвал)</option>
                                            <option value="content" {{ old('type', $duplicateData['type'] ?? '') == 'content' ? 'selected' : '' }}>Content (Контент)</option>
                                            <option value="gallery" {{ old('type', $duplicateData['type'] ?? '') == 'gallery' ? 'selected' : '' }}>Gallery (Галерея)</option>
                                            <option value="testimonials" {{ old('type', $duplicateData['type'] ?? '') == 'testimonials' ? 'selected' : '' }}>Testimonials (Отзывы)</option>
                                            <option value="pricing" {{ old('type', $duplicateData['type'] ?? '') == 'pricing' ? 'selected' : '' }}>Pricing (Цены)</option>
                                            <option value="team" {{ old('type', $duplicateData['type'] ?? '') == 'team' ? 'selected' : '' }}>Team (Команда)</option>
                                            <option value="blog" {{ old('type', $duplicateData['type'] ?? '') == 'blog' ? 'selected' : '' }}>Blog (Блог)</option>
                                            <option value="other" {{ old('type', $duplicateData['type'] ?? '') == 'other' ? 'selected' : '' }}>Other (Другое)</option>
                                        </select>
                                        @error('type')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <div class="input-group">
                                            <select class="form-select @error('category') is-invalid @enderror" 
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{ $category }}" {{ old('category', $duplicateData['category'] ?? '') == $category ? 'selected' : '' }}>
                                                        {{ $category }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <input type="text" 
                                               ...
                                               id="category" 
                                               name="category" 
                                               value="{{ old('category', $duplicateData['category'] ?? '') }}" 
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="number" 
                                               ...
                                               id="sort_order" 
                                               name="sort_order" 
                                               value="{{ old('sort_order', $duplicateData['sort_order'] ?? 0) }}" 
                                               min="0">
                                        @error('sort_order')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                              id="description" 
                                              name="description" 
                                              rows="3">{{ old('description', $duplicateData['description'] ?? '') }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                <textarea class="form-control @error('html_content') is-invalid @enderror" 
                                          ...
                                          name="html_content" 
                                          rows="15" 
                                          required>{{ old('html_content', $duplicateData['html_content'] ?? '') }}</textarea>
                                @error('html_content')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                <textarea class="form-control @error('css_content') is-invalid @enderror" 
                                          id="css_content" 
                                          name="css_content" 
                                          rows="10">{{ old('css_content', $duplicateData['css_content'] ?? '') }}</textarea>
                                @error('css_content')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="form-text text-muted">
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                <textarea class="form-control @error('js_content') is-invalid @enderror" 
                                          id="js_content" 
                                          name="js_content" 
                                          rows="8">{{ old('js_content', $duplicateData['js_content'] ?? '') }}</textarea>
                                @error('js_content')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="form-text text-muted">
                            ...
                            <div class="card-body">
                                <div class="form-check">
                                    <input class="form-check-input" 
                                           ...
                                           id="is_active" 
                                           name="is_active" 
                                           {{ old('is_active', $duplicateData['is_active'] ?? true) ? 'checked' : '' }}>
